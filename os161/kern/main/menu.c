...
common_prog(int nargs, char **args)
{
	...
	int ret;
	syscall_waitpid(temp->pID, 0, 0, &ret);
	return 0;
}
...
//For user command df # on/off
static
int
cmd_df(int nargs, char **args){

	u_int32_t mod = atoi(args[1]);
	if(nargs != 3){
		kprintf("Usage: df nr on/off\n");
	}
	else if(mod <= 12){
		if(strcmp(args[2], "on") == 0){
			int bitOrVal = 1 << (mod-1);
			dbflags = (dbflags | bitOrVal);
			} 
...
//
// Command table.
//DEBUG(DB_THREADS, "DEBUG message 3 is here", 0);
static struct {
	const char *name;
